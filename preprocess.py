...
def main():
    ...
    if os.path.isfile(textgrid_name):
        shutil.move('{}'.format(textgrid_name), out_dir)

    if not os.path.exists(os.path.join(out_dir, textgrid_name.replace(".zip", ""))):
        with zipfile.ZipFile(out_dir+'/'+textgrid_name,'r') as zip_ref:
    ...
    if "kss" in hp.dataset:
        ...
        if "v.1.4" in meta:
            if not os.path.exists(os.path.join(in_dir, "wavs_bak")):
                os.makedirs(os.path.join(in_dir, "wavs"))
                shutil.move(os.path.join(in_dir,'../',meta), os.path.join(in_dir))
                for i in range(1, 5) : 
                    shutil.move(os.path.join(in_dir, str(i)), os.path.join(in_dir, "wavs"))
                shutil.move(os.path.join(in_dir, 'wavs'), os.path.join(in_dir, 'wavs_bak'))
                os.makedirs(os.path.join(in_dir, "wavs"))
        ...
        if not os.path.exists(os.path.join(in_dir, "wavs_bak")):
            os.makedirs(os.path.join(in_dir, "wavs"))
            shutil.move(os.path.join(in_dir,'../',meta), os.path.join(in_dir))
            shutil.move(os.path.join(in_dir, str(1)), os.path.join(in_dir, "wavs"))
            shutil.move(os.path.join(in_dir, 'wavs'), os.path.join(in_dir, 'wavs_bak'))
            os.makedirs(os.path.join(in_dir, "wavs"))

        train, val = other.build_from_path(in_dir, out_dir, meta)
